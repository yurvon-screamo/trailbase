...
async fn test_oauth_login_flow_with_form_post_callback() {
  ...
  let internal_redirect = callback::post_callback_from_external_auth_provider(
    State(state.clone()),
    Path(TestOAuthProvider::NAME.to_string()),
    Extension(HasRoot(false)),
    cookies.clone(),
    Form(callback::AuthQuery {
      state: auth_query.state.clone(),
      code: auth_query.code_challenge.clone(),
    }),
  )
...
async fn test_oauth_callback_rejects_invalid_state() {
  ...
  let result = callback::post_callback_from_external_auth_provider(
    State(state.clone()),
    Path(TestOAuthProvider::NAME.to_string()),
    Extension(HasRoot(false)),
    cookies.clone(),
...
async fn test_oauth_form_post_callback_via_http_router() {
  ...
  // Trigger a login: the test server's cookie jar stores the OAuth state cookie and
  // forwards it with subsequent requests.
  let redirect_uri = format!("{site_url}/login-success-welcome");
  ...
  // callback is reached via a real HTTP POST through the router, forwarding the state
  // cookie like a browser would.
